...
> Método construído a partir da análise de dados reais de 3 referências (Singularidade, Ciência Todo Dia, MrBeast) + diagnóstico dos 7 primeiros vídeos do canal. Cada regra está fundamentada em padrões observados, não em opinião.

---

## Visão Geral

O canal tem um sistema de produção de roteiros (prompt v8) mais sofisticado que 90% dos concorrentes. O problema não é a qualidade do conteúdo — é a **embalagem** e a **estratégia de distribuição**.

Os 3 gargalos que explicam 646 views em 7 vídeos:

| Gargalo | Evidência | Solução |
|---|---|---|
| **Títulos** descritivos, não magnéticos | "Vacina Personalizada Contra o Câncer" = 43 views | Checklist de título (Seção 2) |
| **Tema repetitivo** (7/7 = IA + câncer) | Algoritmo não sabe para quem distribuir | Matriz de temas (Seção 1) |
| **Sem estratégia Short → Longo** | Shorts fazem 159 views mas 0 conversão | Funil de conteúdo (Seção 5) |
...
O Singularidade nunca repetiu tema em 20 vídeos. Ciência Todo Dia nunca repete em 20 consecutivos. Seu canal tem 7/7 sobre IA + câncer/saúde. O algoritmo interpreta isso como conteúdo de nicho ultra-restrito e limita a distribuição.
...
| IA + Robótica | Médio | "O Robô que Aprendeu a Operar Sozinho" | Último vídeo do canal já explorou isso |
...
### Teste de tema: 3 perguntas antes de produzir

Antes de iniciar qualquer vídeo, o tema deve passar por estas 3 perguntas:
...
### Processo: gerar 10, escolher 1

Para cada vídeo:
1. Gere 10 títulos usando as 6 fórmulas acima (o prompt v8 já faz isso)
2. Aplique o checklist em cada um
3. Elimine os que falham em qualquer item
4. Dos restantes, escolha o que você mais gostaria de clicar se visse no feed

### O que mudar no prompt v8

O prompt v8 atual prioriza estruturas de título como `[GATILHO em CAPS]: [Fato]` (ex: "CONFIRMADO: IA Detecta Câncer de Mama"). Os dados mostram que esse formato funciona como manchete de notícia, mas **não como premissa de filme**. Os títulos dos 7 vídeos publicados seguem esse padrão e são os de pior performance.

**Atualização recomendada:** substituir as 5 estruturas de título do prompt v8 pelas 6 fórmulas acima. Manter o limite de 90 caracteres e a geração de 10 opções, mas trocar os templates.

---

## 3. Estrutura Narrativa

### A narrativa atual está correta — falta escalonamento

O prompt v8 já tem uma estrutura sólida (hook → contexto → desenvolvimento em blocos → loops → CTA). O que falta é **escalonamento progressivo** dentro do desenvolvimento.
...
O prompt v8 já pede blocos com "Conclusão Primeiro, Jornada Depois" dentro de cada bloco. O escalonamento é a camada ACIMA dos blocos — a progressão ENTRE eles.
...
O prompt v8 atual mira 12-15 min nos longos. Ajustar para 13-17 min (expandir o desenvolvimento, não o hook/CTA).

---

## 4. Cadência de Publicação

### A cadência atual é insustentável e ineficiente

7 vídeos em 2 semanas = ~3.5/semana. Mas nenhum desses vídeos teve tempo de ser otimizado em título, thumbnail ou narrativa. O resultado: 92 views/vídeo.
...
2. **Thumbnail**: produzir 2-3 variações, escolher a melhor (1 hora)
3. **Narrativa**: revisar o escalonamento entre blocos (30 min)
4. **SEO**: tags extensas (15-20), descrição otimizada, legendas (30 min)
...
Os Shorts geram views (média 159) mas não convertem para os vídeos longos (média 5 views). Os Shorts funcionam como conteúdo isolado — não como porta de entrada para o canal.
...
| **Views por Short** | >500 até Short 20 | YouTube Studio | Shorts são o motor de crescimento |

### Metas progressivas (primeiros 3 meses)

| Período | Meta principal | Meta secundária |
|---|---|---|
| **Mês 1** (vídeos 8-11) | >200 views/longo, >500 views/Short | 1 comentário por vídeo |
| **Mês 2** (vídeos 12-15) | >500 views/longo, >1K views/Short | 50 inscritos |
| **Mês 3** (vídeos 16-19) | >1K views/longo, >3K views/Short | 200 inscritos, 1 vídeo >5K views |

Essas metas são baseadas na progressão observada no Singularidade (que atingiu 1,690 subs em 2 meses com 20 vídeos) ajustadas para uma cadência menor.
...
## 7. Atualizações Recomendadas no Prompt v8

O prompt v8 é forte mas precisa de ajustes baseados nos dados coletados:

| Área | Situação atual | Ajuste recomendado |
|---|---|---|
| **Fórmulas de título** | 5 estruturas tipo manchete de notícia ("CONFIRMADO:", "REVELADO:") | Substituir pelas 6 fórmulas da Seção 2 (pergunta existencial, X vs Y, contradição, número impossível, descoberta+consequência, E se...) |
| **Duração alvo** | 12-15 min | Ajustar para 13-17 min |
| **Escalonamento** | Não mencionado explicitamente | Adicionar instrução de escalonamento progressivo entre blocos (Seção 3) |
| **Campos variáveis** | Não incluem sub-nicho | Adicionar campo "SUB-NICHO" com a matriz de temas (Seção 1) para forçar diversidade |
| **CTA do Short** | "siga para mais" (genérico) | Instruir CTA específico conectado ao longo correspondente |
| **Gatilhos de título** | CONFIRMADO, REVELADO, DESCOBRIRAM | Adicionar: pergunta existencial, negação/contradição, formato X vs Y |
...
4. **Escalonamento progressivo entre blocos (tensão sempre sobe)**
5. **1 longo a cada 2 semanas + 4 Shorts por ciclo**
6. **4 Shorts por longo: teaser, standalone, lançamento, reprise**
7. **CTA específico, nunca genérico**
8. **13-17 min nos longos, 45-60s nos Shorts**
9. **Like ratio >3%, CTR >5%, Retenção >40%**
10. **Revisão mensal baseada em dados, não em intuição**
